#This script generates the Dose-response curves and EC50 values for the individual peptide-receptor pairs of the Thiel et al paper on Nematostella GPCR deorphanisation
...
Okabe_Ito <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", 
               "#CC79A7", "#000000")
# read data ---------------------------------------------------------------
AllGPCR <- vroom("data/Nvec_Dose_response_assays.csv")
...
for (i in ListREceptors) {
  ...
  for (k in ListPeptides) {
    ...
    if (nrow(SubsetRecPep) > 0) {
      ...
      df <- rbind(df, dfrow)
      write.csv(df, file = "supplements/EC50s_and_slope.csv", sep = ",", row.names = FALSE);
    }
  }
}

#histogram of min EC50 values (for each receptor only the lowest value)
as_tibble(df[2:43, ]) %>%                      # Specify data frame
  group_by(Receptor) %>%
